...
void rgbrec_update_current_channel(uint8_t channel) {
    ...
    offset = channel * sizeof(rgbrec_buffer);
...
uint8_t record_color_read_data(void) {
    ...
    uint8_t hs_c = 0;
    // confinfo starts after rgbrec data, +4 offset for other confinfo fields
    uint32_t offset = EECONFIG_RGBREC_USE_SIZE + 4 + hs_mode;
...
void record_color_hsv(bool status) {
    ...
    // confinfo starts after rgbrec data, +4 offset for other confinfo fields
    uint32_t offset = EECONFIG_RGBREC_USE_SIZE + 4 + find_index();
